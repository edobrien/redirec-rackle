...
<div ng-cloak ng-controller="SurveyController" class="px-2 py-4">
    <div class="row mb-3">
        <div class="col-md-8">
            <h4 class="font-weight-bold text-blue">Surveys</h4> 
        </div>
        <div class="col-md-4">
            <button ng-click="addSurvey()" title="Add Survey" title="Add Survey" class=" btn btn-circle btn-mn bg-darkblue pull-right rounded-0">
    ...
    <div class="responsive-table">
        ...
        <table id="survey-listing" class="table table-striped table-responsive-sm table-responsive-md  table-responsive-lg" width="100%" cellspacing="0">
            <thead>
                <tr>
        ...
        <div id="survey-modal" class="modal fade" role="dialog">
            <div class="modal-dialog modal-lg">
                <div class="modal-content rounded-0">
                    <form class="cmxform" ng-submit="surveySubmit(form_data)" ng-model="form_data" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title font-weight-bold">Survey</h4>
                        </div>
                        <div class="modal-body">
                            <div class="form-row">
                                ...
                                <div class="form-group form-animate-text col-md-6">
                                    <label class="mb-0">Hyperlink Text</label>
                                    <input type="text" class="form-text" ng-model="form_data.title" required>
                                </div>
                                <div class="form-group form-animate-text col-md-6">
                                    <label class="mb-0">Hyperlink</label>
                                    <input type="text" class="form-text" ng-model="form_data.url" required>
                                </div>
                                <div class="form-group form-animate-text col-md-6">
                                    <label class="mb-0">Ordering</label>
                                    <input type="number" class="form-text" ng-model="form_data.ordering">
                                </div>
                                <div class="form-group form-animate-checkbox col-md-6">
                                    ...
                                    <label class="switch">
                                        <input type="checkbox" class="switch-input checkbox"
                                            ng-true-value="'<?php echo \App\Survey::FLAG_YES; ?>'"
                                            ng-false-value="'<?php echo \App\Survey::FLAG_NO; ?>'"
            ...
            <div class="modal-dialog">
              <div class="modal-content rounded-0">
                ...
                <div class="modal-footer border-0">
                  ...
                  <button type="button" ng-if="showDelete" ng-click="deleteSurveyConfirmed()" 
...
<script type="text/javascript">
    ...
    app.controller('SurveyController', function ($scope, $http, $compile) {
        ...
        $scope.surveySubmit = function(form_data){
            $(".bg_load").show();
            $scope.modalErrors = null;
            var url = 'survey/add-update-url';
        ...
        $scope.editSurvey = function(survey_id){
            $(".bg_load").show();
            $scope.modalErrors = null;
            var url = 'survey/get-info/' + survey_id;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $("#survey-modal").modal('show');
                    $scope.form_data  = response.data.survey;
                } else {
                    var errors = [];
                    $.each(response.data.errors, function (key, value) {
                        errors.push(value);
        ...
        $scope.deleteSurvey = function(survey_id){
            $scope.reference_to_delete = survey_id;
            $scope.messageToshow = 'You are going to remove this record. Are you sure?';
            $scope.showDelete = true;
            $("#delete-confirm").modal('show');
        }

        $scope.deleteSurveyConfirmed = function(){
            $(".bg_load").show();
            var url = 'survey/delete/'+$scope.reference_to_delete;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
