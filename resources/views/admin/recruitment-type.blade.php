...
<div ng-cloak ng-controller="RecruitmentController" class="px-2 py-4">
    <div class="row mb-3">
        <div class="col-md-8">
            <h4 class="font-weight-bold text-blue">Recruitment Types</h4> 
        </div>
        <div class="col-md-4">
            <button ng-click="addRecruitment()" title="Add Recruitment" title="Add Recruitment" class=" btn btn-circle btn-mn bg-darkblue pull-right rounded-0">
        ...
        <div id="type-modal" class="modal fade" role="dialog">
            <div class="modal-dialog modal-lg">
                <div class="modal-content rounded-0">
                    <form class="cmxform" ng-submit="typeSubmit(form_data)" ng-model="form_data" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title font-weight-bold">Recruitment Type</h4>
                        </div>
                                ...
                                <div class="form-group form-animate-checkbox col-md-6">
                                    ...
                                    <label class="switch">
                                        <input type="checkbox" class="switch-input checkbox"
                                            ng-true-value="'<?php echo \App\RecruitmentType::FLAG_YES; ?>'"
                                            ng-false-value="'<?php echo \App\RecruitmentType::FLAG_NO; ?>'"
            ...
            <div class="modal-dialog">
              <div class="modal-content rounded-0">
                ...
                <div class="modal-footer border-0">
                  ...
                  <button type="button" ng-if="showDelete" ng-click="deleteTypeConfirmed()" 
...
<script type="text/javascript">
    ...
    app.controller('RecruitmentController', function ($scope, $http, $compile) {
        ...
        $scope.typeSubmit = function(form_data){
            $(".bg_load").show();
            $scope.modalErrors = null;
            var url = 'recruitment-type/add-update-type';
            $http.post(url,form_data).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $("#type-modal").modal('hide');
                    $scope.successMessage = response.data.message;
                    $scope.listRecruitments();
                } else {
                    var errors = [];
        ...
        $scope.editType = function(type_id){
            $(".bg_load").show();
            $scope.modalErrors = null;
            var url = 'recruitment-type/get-info/' + type_id;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $("#type-modal").modal('show');
                    $scope.form_data  = response.data.type;
                } else {
                    var errors = [];
                    $.each(response.data.errors, function (key, value) {
                        errors.push(value);
        ...
        $scope.deleteType = function(type_id){
            $scope.reference_to_delete = type_id;
            $scope.messageToshow = 'You are going to remove this record. Are you sure?';
            $("#delete-confirm").modal('show');
            var url = 'recruitment-type/can-delete-type/'+type_id;
            $http.get(url).then(function (response) {
                if (response.data.status != 'SUCCESS') {
        ...
        $scope.deleteTypeConfirmed = function(){
            $(".bg_load").show();
            var url = 'recruitment-type/delete/'+$scope.reference_to_delete;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $scope.successMessage = response.data.message;
                    $scope.listRecruitments();
        ...
        $scope.listRecruitments = function(){
            $('#recruitment-listing').DataTable({
                processing: true,
                stateSave: true,
                serverSide: true,
                destroy: true,
                ajax: {
                    url: 'recruitment-type/list-types',
                },
                columns: [
                    {data: 'name'},
