...
<div ng-cloak ng-controller="SectorController" class="px-2 py-4">
    <div class="row mb-3">
        <div class="col-md-8">
            <h4 class="font-weight-bold text-blue">Sectors</h4> 
        </div>
        <div class="col-md-4">
            <button ng-click="addSector()" title="Add Sector" title="Add Sector" class=" btn btn-circle btn-mn bg-darkblue pull-right rounded-0">
    ...
    <div class="responsive-table">
        ...
        <table id="sector-listing" class="table table-striped table-responsive-sm" width="100%" cellspacing="0">
            <thead>
                <tr>
                    <th>Sector Name</th>
                    <th>Type</th>
        ...
        <div id="sector-modal" class="modal fade" role="dialog">
            <div class="modal-dialog modal-lg">
                <div class="modal-content rounded-0">
                    <form class="cmxform" ng-submit="sectorSubmit(form_data)" ng-model="form_data" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title font-weight-bold">Sector</h4>
                        </div>
                                ...
                                <div class="form-group form-animate-checkbox col-md-6">
                                    ...
                                    <label class="switch">
                                        <input type="checkbox" class="switch-input checkbox"
                                            ng-true-value="'<?php echo \App\Sector::FLAG_YES; ?>'"
                                            ng-false-value="'<?php echo \App\Sector::FLAG_NO; ?>'"
            ...
            <div class="modal-dialog">
              <div class="modal-content rounded-0">
                ...
                <div class="modal-footer border-0">
                  ...
                  <button type="button" ng-if="showDelete" ng-click="deleteSectorConfirmed()" 
...
<script type="text/javascript">
    ...
    app.controller('SectorController', function ($scope, $http, $compile) {
        ...
        $scope.sectorSubmit = function(form_data){
            $(".bg_load").show();
            $scope.modalErrors = null;
            var url = 'sector/add-update-sector';
            $http.post(url,form_data).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $("#sector-modal").modal('hide');
        ...
        $scope.editSector = function(sector_id){
            $(".bg_load").show();
            $scope.modalErrors = null;
            var url = 'sector/get-info/' + sector_id;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $("#sector-modal").modal('show');
                    $scope.form_data  = response.data.sector;
                } else {
                    var errors = [];
                    $.each(response.data.errors, function (key, value) {
                        errors.push(value);
        ...
        $scope.deleteSector = function(sector_id){
            ...
            var url = 'sector/can-delete-sector/'+sector_id;
            $http.get(url).then(function (response) {
                if (response.data.status != 'SUCCESS') {
        ...
        $scope.deleteSectorConfirmed = function(){
            $(".bg_load").show();
            var url = 'sector/delete/'+$scope.reference_to_delete;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $scope.successMessage = response.data.message;
                    $scope.listSectors();
        ...
        $scope.listSectors = function(){
            $('#sector-listing').DataTable({
                processing: true,
                stateSave: true,
                serverSide: true,
                destroy: true,
                ajax: {
                    url: 'sector/list-sectors',
                },
                columns: [
                    {data: 'name', name:'name'},
