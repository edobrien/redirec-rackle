...
<div ng-cloak ng-controller="InterviewGuideSectionController" class="px-2 py-4">
    <div class="row mb-3">
        <div class="col-md-8">
            <h4 class="font-weight-bold text-blue">Interview Guide Sections</h4> 
        </div>
        <div class="col-md-4">
            <button ng-click="addInterviewSection()" title="Add Interview Section" 
                    title="Add Interview Section" 
                    class=" btn btn-circle btn-mn bg-darkblue pull-right rounded-0">
    ...
    <div class="responsive-table">
        ...
        <table id="section-listing" class="table table-striped table-responsive-sm table-responsive-md  table-responsive-lg" width="100%" cellspacing="0">
            <thead>
                <tr>
                    <th>Section Name</th>
                    <th>IsActive</th>
        ...
        <div id="interview-section-modal" class="modal fade" role="dialog">
            <div class="modal-dialog modal-lg">
                <div class="modal-content rounded-0">
                    <form class="cmxform" ng-submit="sectionSubmit(form_data)" ng-model="form_data" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title font-weight-bold">Interview Guide Section</h4>
                        </div>
...
<script type="text/javascript">
    ...
    app.controller('InterviewGuideSectionController', function ($scope, $http, $compile) {

        $scope.addInterviewSection = function(){
            $scope.form_data = $scope.modalErrors  = null;
            $("#interview-section-modal").modal('show');
        }

        $scope.sectionSubmit = function(form_data){
            $(".bg_load").show();
            $scope.modalErrors = null;
            var url = 'interview-guide-sections/add-update-sections';
            $http.post(url,form_data).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $("#interview-section-modal").modal('hide');
                    $scope.successMessage = response.data.message;
                    $scope.listInterviewGuideSections();
                } else {
                    var errors = [];
                    ...
                    $("#interview-section-modal").animate({ scrollTop: 0 }, "slow");
                }
            }).finally(function(){
                $(".bg_load").hide();
            });
        ...
        $scope.editSection = function(section_id){
            $(".bg_load").show();
            $scope.modalErrors = null;
            var url = 'interview-guide-sections/get-info/' + section_id;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $("#interview-section-modal").modal('show');
                    $scope.form_data  = response.data.interview_section;
                } else {
                    var errors = [];
                    $.each(response.data.errors, function (key, value) {
                        errors.push(value);
        ...
        $scope.deleteSectionConfirmed = function(){
            $(".bg_load").show();
            var url = 'interview-guide-sections/delete/'+$scope.reference_to_delete;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
        ...
        $scope.listInterviewGuideSections = function(){
            $('#section-listing').DataTable({
                ...
                destroy: true,
                ajax: {
                    url: 'interview-guide-sections/list-sections',
                },
                columns: [
                    {data: 'title'},
