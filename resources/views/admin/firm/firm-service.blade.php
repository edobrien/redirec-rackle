...
<div ng-cloak ng-controller="FirmServiceController" class="px-2 py-4">
    <div class="row mb-3">
        <div class="col-md-8">
            <h4 class="font-weight-bold text-blue">Firm Services</h4> 
        </div>
        <div class="col-md-4">
            <button ng-click="addFirmService()" title="Add Firm Service" title="Add Firm Service" class=" btn btn-circle btn-mn bg-darkblue pull-right rounded-0">
    ...
    <div class="responsive-table">
        ...
        <table id="firm-service-listing" class="table table-striped table-responsive-sm" width="100%" cellspacing="0">
            <thead>
                <tr>
                    <th>Firm Name</th>
                    <th>Service Name</th>
        ...
        <div id="firm-service-modal" class="modal fade" role="dialog">
            <div class="modal-dialog modal-lg">
                <div class="modal-content rounded-0">
                    <form class="cmxform" ng-submit="firmServiceSubmit(form_data)" ng-model="form_data" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title font-weight-bold">Firm Service</h4>
                        </div>
                        <div class="modal-body">
                            <div class="form-row">
                                ...
                                <div class="form-group form-animate-text col-md-6">
                                    ...
                                    <select class="form-control" ng-model="form_data.service_id" 
                                            ng-options="service.id as service.name for service in services" required>
                                            <option value="">Select</option>
                                    </select>
                                </div>
                                <div class="form-group form-animate-checkbox col-md-6">
                                    ...
                                    <label class="switch">
                                        <input type="checkbox" class="switch-input checkbox"
                                            ng-true-value="'<?php echo \App\FirmService::FLAG_YES; ?>'"
                                            ng-false-value="'<?php echo \App\FirmService::FLAG_NO; ?>'"
            ...
            <div class="modal-dialog">
              <div class="modal-content rounded-0">
                ...
                <div class="modal-footer border-0">
                  ...
                  <button type="button" ng-if="showDelete" ng-click="deleteFirmServiceConfirmed()" 
...
<script type="text/javascript">
    ...
    app.controller('FirmServiceController', function ($scope, $http, $compile) {

        $scope.addFirmService = function(){
            $scope.form_data = $scope.modalErrors  = null;
            $("#firm-service-modal").modal('show');
            $scope.getActiveServices();
            $scope.getActiveFirms();
        }

        $scope.firmServiceSubmit = function(form_data){
            $(".bg_load").show();
            $scope.modalErrors = null;
            var url = 'firm-service/add-update-firm-service';
            $http.post(url,form_data).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $("#firm-service-modal").modal('hide');
                    $scope.successMessage = response.data.message;
                    $scope.listFirmServices();
                } else {
                    var errors = [];
                    ...
                    $("#firm-service-modal").animate({ scrollTop: 0 }, "slow");
                }
            }).finally(function(){
                $(".bg_load").hide();
            });
        ...
        $scope.editFirmService = function(firm_service_id){
            $(".bg_load").show();
            $scope.modalErrors = null;
            $scope.getActiveServices();
            $scope.getActiveFirms();
            var url = 'firm-service/get-info/' + firm_service_id;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $("#firm-service-modal").modal('show');
                    $scope.form_data  = response.data.firm_service;
                } else {
                    var errors = [];
                    $.each(response.data.errors, function (key, value) {
                        errors.push(value);
        ...
        $scope.deleteFirmService = function(firm_service_id){
        ...
        $scope.deleteFirmServiceConfirmed = function(){
            $(".bg_load").show();
            var url = 'firm-service/delete/'+$scope.reference_to_delete;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $scope.successMessage = response.data.message;
                    $scope.listFirmServices();
        ...
        $scope.listFirmServices = function(){
            $('#firm-service-listing').DataTable({
                processing: true,
                stateSave: true,
                serverSide: true,
                destroy: true,
                ajax: {
                    url: 'firm-service/list-firm-services',
                },
                columns: [
