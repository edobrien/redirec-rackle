...
<div ng-cloak ng-controller="LocationController" class="px-2 py-4">
    <div class="row mb-3">
        <div class="col-md-8">
            <h4 class="font-weight-bold text-blue">Hire Locations</h4> 
        </div>
        <div class="col-md-4">
                ...
                <div class="modal-content rounded-0">
                    <form class="cmxform" ng-submit="locationSubmit(form_data)" ng-model="form_data" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title font-weight-bold">Hire Location</h4>
                        </div>
                                ...
                                <div class="form-group form-animate-checkbox col-md-6">
                                    ...
                                    <label class="switch">
                                        <input type="checkbox" class="switch-input checkbox"
                                            ng-true-value="'<?php echo \App\HireLocation::FLAG_YES; ?>'"
                                            ng-false-value="'<?php echo \App\HireLocation::FLAG_NO; ?>'"
...
<script type="text/javascript">
    ...
    app.controller('LocationController', function ($scope, $http, $compile) {
        ...
        $scope.locationSubmit = function(form_data){
            ...
            var url = 'hire-location/add-update-location';
        ...
        $scope.editLocation = function(location_id){
            ...
            $scope.modalErrors = null;
            $scope.getActiveRegions();
            var url = 'hire-location/get-info/' + location_id;
        ...
        $scope.deleteLocation = function(location_id){
            ...
            var url = 'hire-location/can-delete-location/'+location_id;
            $http.get(url).then(function (response) {
                if (response.data.status != 'SUCCESS') {
        ...
        $scope.deleteLocationConfirmed = function(){
            $(".bg_load").show();
            var url = 'hire-location/delete/'+$scope.reference_to_delete;
            $http.get(url).then(function (response) {
                if (response.data.status == 'SUCCESS') {
                    $scope.successMessage = response.data.message;
                    $scope.listLocations();
        ...
        $scope.listLocations = function(){
            $('#location-listing').DataTable({
                ...
                destroy: true,
                ajax: {
                    url: 'hire-location/list-locations',
                },
                columns: [
                    {data: 'name', name:'name'},
